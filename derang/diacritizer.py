...
class Diacritizer:
    ...
    def diacritize_text(self, sentences: list[str], batch_size=_missing):
        ...
        for batch in tqdm(batches, total=len(batches)):
            sents, infer_time = self._do_diacritize_textb(batch)
    ...
    def _do_diacritize_text(self, sentences: list[str]):
        sentences = [persian_cleaner(sent) for sent in sentences]
        parsed_sents = [extract_haraqat(sent) for sent in sentences]
        
        char_seqs = [self.text_encoder.input_to_sequence(ps[1]) for ps in parsed_sents]
        char_seqs = list(filter(None, char_seqs))
        lengths = [len(seq) for seq in char_seqs]
        max_len = max(lengths)
        
        if len(char_seqs) > 1:
            char_seqs = [list(padded(seq, fillvalue=self.input_pad_id, n=max_len)) for seq in char_seqs]
        ...
        for length, src, prediction in zip(input_lengths, char_inputs, predictions):
            ...
            sentence = self.text_encoder.combine_text_and_diacritics(list(src), list(prediction))
...
class TorchDiacritizer(Diacritizer):
    ...
    def diacritize_batch(self, char_inputs, input_lengths):
        ...
        return (indices.detach().cpu().numpy(), logits.detach().cpu().numpy())
    

class OnnxDiacritizer(Diacritizer):
    def __init__(self, *args, onnx_model, **kwargs) -> None:
        super().__init__(*args, **kwargs)
